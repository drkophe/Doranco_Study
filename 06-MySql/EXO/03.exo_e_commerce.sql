...
# EXO :afficher les produits du plus cher au moins cher ( il faut utiliser ORDER BY )   ( E_COMMERCE)
SELECT * FROM produit ORDER BY prix DESC;

# EXO : afficher la liste des produits qui sont dans le departement 'Toys'  (E_COMMERCE)
SELECT * FROM produit WHERE departement = 'Toys';

# EXO  :  ecriver une requette qui permet d'afficher les commandes payé ( E_COMMERCE)
SELECT * FROM commande WHERE paye = true;

# EXO : ecriver une requette qui permet d'afficher les produits qui ont un poids entre 3 et 30 (E_COMMERCE)
SELECT * FROM produit WHERE poids BETWEEN 3 AND 30;


#EXO : ecriver une requette qui permet d'afficher les utilisateurs qui un prénom qui commence par la lettre 'S'  ( E_COMMERCE)
SELECT * FROM utilisateur WHERE nom LIKE 'S%';

#EXO : Selectionner les 10 premier produits
SELECT * FROM produit WHERE id BETWEEN 1 AND 10;

SELECT * FROM produit ORDER BY id ASC LIMIT 10 ;

#EXO  : selectionner les produits du 10eme au 20eme
SELECT * FROM produit WHERE id BETWEEN 10 AND 20;

#EXO : Selectionner les 10 derniers utilisateurs
SELECT * FROM utilisateur WHERE id BETWEEN (SELECT MAX(id)-10 FROM utilisateur) AND (SELECT MAX(id) FROM utilisateur);

SELECT * FROM utilisateur ORDER BY id DESC LIMIT 10; 
...
-- WHERE id_utilisateur BETWEEN 
-- (SELECT MAX(utilisateur.id)-10 FROM utilisateur) 
-- AND 
-- (SELECT MAX(utilisateur.id) FROM utilisateur) 
-- AND 
-- paye=true;
...
#SELECT * FROM utilisateur WHERE id BETWEEN (SELECT MAX(id)-10 FROM utilisateur JOIN commande ) AND (SELECT MAX(id) FROM utilisateur);

#EXO : selectionner les 5 produits les moins cher
SELECT * FROM produit WHERE prix BETWEEN (SELECT MIN(prix) FROM produit) AND (SELECT MIN(prix)+32 FROM produit);

SELECT * FROM produit ORDER BY prix ASC LIMIT 5;

#EXO : selectionner les 3 produits les plus cher
SELECT * FROM produit WHERE prix BETWEEN (SELECT MAX(prix) -10 FROM produit) AND (SELECT MAX(prix) FROM produit);

SELECT * FROM produit ORDER BY prix DESC LIMIT 3;

# EXO trouver le nom et le prix des  produits qui sont plus cher que tout les produits qui sont le departement 'toys'
SELECT
nom,
prix
...
#EXO trouver le prix moyen de tout les produits
SELECT 
SUM(prix) AS prix_total,
COUNT(prix) AS nombre_de_produit,
(SUM(prix)/COUNT(prix)) AS moyenne

FROM produit;

#EXO trouver le nombre de commande effectuer par chaque utilisateur
SELECT
nom,
COUNT(commande.id) AS nombre_de_commande

FROM utilisateur
JOIN commande ON commande.id_utilisateur = utilisateur.id

GROUP BY nom
ORDER BY nombre_de_commande DESC;


# EXO : trouver le prix moyen des produit pour chaque department
...
#########
#verification de l'exo d'en haut
SELECT
departement,
prix
...
INSERT INTO utilisateur (prenom, nom) 
VALUES 
  ('Iva', 'Lindgren'),
  ('Ignatius', 'Johns'),
  ('Jannie', 'Boehm'),
...
INSERT INTO produit (nom, departement, prix, poids)
VALUES
...
INSERT INTO commande (id_utilisateur, id_produit, paye)
VALUES
  (41, 100, true),
  (27, 99, false),
  (50, 72, false),
